...
public class HollisterSearchController implements DrisqController {

	public static final String FXML_RSC = "rsc/HollisterSearch.fxml";
	...
	public static HollisterSearchController newInstance(Window owner, String title) {
		Stage stage = FxUtil.newStage(owner, FXML_RSC, HollisterSearchController.class, title);
		return (HollisterSearchController) FxUtil.getController(stage.getScene().getRoot());
	}
	...
	private String _launchFindButton() {
		...
		String hollisterQuery = ("SELECT [Product Description], Quantity, Available FROM OurProducts WHERE [Product Type] "
				+ productQuery + " AND Sizes " + sizeQuery + " AND Colour " + colourQuery + "AND Gender " + genderQuery
				+ "AND Brands = 'Hollister'");

		if (!minPrice.isEmpty() && !maxPrice.isEmpty()) {
			hollisterQuery += " AND Price BETWEEN '" + minPrice + "' AND '" + maxPrice + "'";
		} else if (!minPrice.isEmpty()) {
			hollisterQuery += " AND Price >= '" + minPrice + "'";
		...
		try {
			controller.initHollisterTable(hollisterQuery);
