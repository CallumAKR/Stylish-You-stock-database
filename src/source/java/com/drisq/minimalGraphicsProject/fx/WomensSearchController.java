...
public class WomensSearchController implements DrisqController {

	public static final String FXML_RSC = "rsc/WomensSearch.fxml";
	...
	public static WomensSearchController newInstance(Window owner, String title) {
		Stage stage = FxUtil.newStage(owner, FXML_RSC, WomensSearchController.class, title);
		return (WomensSearchController) FxUtil.getController(stage.getScene().getRoot());
	}
	...
	private String _launchFindButton() {
		...
		String womensQuery = ("SELECT [Product Description], Brands, Quantity, Available FROM OurProducts WHERE [Product Type] "
				+ productQuery + " AND Sizes " + sizeQuery + " AND Colour " + colourQuery + "AND Brands " + brandsQuery
				+ "AND Gender = 'Ladies'");

		if (!minPrice.isEmpty() && !maxPrice.isEmpty()) {
			womensQuery += " AND Price BETWEEN '" + minPrice + "' AND '" + maxPrice + "'";
		} else if (!minPrice.isEmpty()) {
			womensQuery += " AND Price >= '" + minPrice + "'";
		...
		try {
			controller.initWomensTable(womensQuery);
